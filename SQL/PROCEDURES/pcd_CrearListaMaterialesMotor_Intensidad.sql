CREATE DEFINER=`root`@`localhost` PROCEDURE `CrearListaMaterialesMotor_Intensidad`(p_fabricante VARCHAR(45), p_potencia FLOAT)
BEGIN  
    ...
    INSERT INTO `subtabla listado materiales` (Referencia)
       WITH tabla_1 AS (
        ...
        LEFT JOIN componentes b 
            ...
            AND b.`Potencia (kW)` >= p_potencia
            AND b.`Potencia (kW)` = (
                SELECT MIN(b2.`Potencia (kW)`)
                FROM componentes b2
                WHERE b2.Tipo = a.TipoComponente
                  AND b2.`Potencia (kW)` >= p_potencia
            )
