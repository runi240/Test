CREATE DEFINER=`root`@`localhost` PROCEDURE `CrearListaMaterialesMotor_Potencia`(p_fabricante VARCHAR(45), p_potencia FLOAT)
BEGIN  
    ...
    INSERT INTO `subtabla listado materiales` (Referencia)
       WITH tabla_1 AS (
        ...
        LEFT JOIN componentes b 
            ...
            AND b.`IntensidadMax (A)` >= p_potencia
            AND b.`IntensidadMax (A)` = (
                SELECT MIN(b2.`IntensidadMax (A)`)
                FROM componentes b2
                WHERE b2.Tipo = a.TipoComponente
                  AND b2.`IntensidadMax (A)` >= p_potencia
            )
